...
) AS kba1 ON (kmd.sender_address_reference = kba1.address_id) AND (kmd.sender_pincode = kba1.pincode)
INNER JOIN (
    SELECT address_id, pincode, city, addressee_name 
    FROM mis_db.ext_mailbkg_booking_address 
    WHERE _peerdb_is_deleted = 0
) AS kba2 ON (kmd.receiver_address_reference = kba2.address_id) AND (kmd.receiver_pincode = kba2.pincode)
INNER JOIN (
