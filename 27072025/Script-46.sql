...
LEFT JOIN LATERAL (
  ...
  WHERE
    ais1.article_number = boc.article_number -- Key change: Filter by outer query's article_number
  ORDER BY
    ais1.art_stg_date DESC
  LIMIT 1 -- Get only the latest
) AS ais
  ON TRUE -- Lateral join doesn't need an ON clause here as the filter is in WHERE
