WITH schedule_data AS (
            SELECT schedule_start_time::text as schedule_start_time, source_facility_id, destination_facility_id
            FROM carriermgmt.schedule 
            WHERE schedule_id = 2698
        ),
        sequence_data AS (
            SELECT COALESCE(MAX(sequence_number), 0) AS max_seq,
                   MAX(departure_time)::TIME AS departure_time,
