SELECT *
    ...
    WHERE article_number = 'RT738876445IN'

explain analyze WITH bag_numbers AS (
    SELECT bag_number, 'CLDI'::text AS type
    FROM trackandtrace.kafka_bag_close_content
    WHERE article_number = 'RT738876445IN'
    UNION
    SELECT bag_number, 'OPOR'::text AS type
...
JOIN bag_numbers bn 
       ON be.bag_number = bn.bag_number
...
WHERE (bn.type = 'CLDI' AND be.event_type IN ('CL','DI'))
   OR (bn.type = 'OPOR' AND be.event_type IN ('OP','OR'))
ORDER BY be.transaction_date;

----------
explain analyze 
WITH relevant_bags AS (
    SELECT bag_number FROM trackandtrace.kafka_bag_close_content WHERE article_number = 'RT738876445IN'
    UNION
    SELECT bag_number FROM trackandtrace.kafka_bag_open_content WHERE article_number = 'RT738876445IN'
)
SELECT
    DATE(be.transaction_date) AS Date,
    to_char(be.transaction_date, 'HH24:MI:SS') AS Time,
