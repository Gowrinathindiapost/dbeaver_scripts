...
--insert into amazon_target_table_dt
    ...
    WHERE cxcm.bulk_customer_id =  2000000992
...
--        AND te.event_date < parseDateTime64BestEffort('2025-07-27 00:00:15.198516')
        AND te.event_date BETWEEN parseDateTime64BestEffort('2025-07-01 00:00:00.000000') AND parseDateTime64BestEffort('2025-07-29 23:59:59.000000')
        AND te.event_code='ITEM_RETURN' MS009937770IN
) AS cxcm;
...
FROM (
    ...
    FROM (
        ...
        FROM mis_db.amazon_target_table_dt
        WHERE bulk_customer_id =  2000000992
         AND event_date BETWEEN parseDateTime64BestEffort('2025-07-29 00:00:00.000000') 
                           AND parseDateTime64BestEffort('2025-07-29 23:59:59.000000')
    ) AS ranked
    WHERE rn = 1
) AS filtered_data
...
LEFT JOIN mis_db.new_customer_tracking_event_mv nctenm ON nctenm.article_number = ncxfcnm.article_number 
WHERE ncxfcnm.bulk_customer_id = 2000000992 
  AND nctenm.event_code IN ('ITEM_RETURN', 'ITEM_REDIRECT')
  AND nctenm.event_date BETWEEN parseDateTime64BestEffort('2025-07-29 00:00:00.000000') 
                            AND parseDateTime64BestEffort('2025-07-29 23:59:59.000000')
