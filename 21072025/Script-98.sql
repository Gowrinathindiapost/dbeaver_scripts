...
CREATE MATERIALIZED VIEW mis_db.mv_new_article_events_tracking_event ON CLUSTER cluster_1S_2R
...
CREATE MATERIALIZED VIEW mis_db.mv_new_bag_events_tracking_event ON CLUSTER cluster_1S_2R
TO mis_db.new_customer_tracking_event_mv
(
...
CREATE MATERIALIZED VIEW mis_db.mv_new_pdmanagement_article_transaction_tracking_event ON CLUSTER cluster_1S_2R
TO mis_db.new_customer_tracking_event_mv
(
...
SELECT
	...
	WHERE cxcm.bulk_customer_id = 1000002954
	    AND parseDateTimeBestEffort(cxcm.booking_date) < now()
	     ...
	     AND parseDateTimeBestEffort('2025-07-16 23:59:57.198516') -- ClickHouse will handle the time.Time object conversion
