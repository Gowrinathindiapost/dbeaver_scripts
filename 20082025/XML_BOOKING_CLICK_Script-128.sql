...
WHERE bulk_customer_id = 3000056014
AND booking_date BETWEEN parseDateTime64BestEffort('2025-08-14 00:00:00.000000') 
                     AND parseDateTime64BestEffort('2025-08-14 23:59:59.000000')
                     select distinct * 
                     FROM mis_db.amazon_target_table_dt_ib
WHERE bulk_customer_id = 1000002954
AND booking_date BETWEEN parseDateTime64BestEffort('2025-08-14 00:00:00.000000') 
                     AND parseDateTime64BestEffort('2025-08-14 23:59:59.000000') AND
    ...
    FROM (
        SELECT 
            *,
            ROW_NUMBER() OVER (PARTITION BY article_number ORDER BY booking_date DESC, booking_time DESC) AS rn
        FROM mis_db.amazon_target_table_dt_ib
        WHERE bulk_customer_id = 3000068453 --1000002954
        AND booking_date BETWEEN parseDateTime64BestEffort('2025-08-14 00:00:00.000000') 
                             AND parseDateTime64BestEffort('2025-08-14 23:59:59.000000')
        AND event_date BETWEEN parseDateTime64BestEffort('2025-08-14 00:00:00.000000') 
                           AND parseDateTime64BestEffort('2025-08-14 23:59:59.000000')
    ) AS ranked
    WHERE rn = 1
) AS filtered_data