...
SELECT
    ...
    emom.office_name AS office_name,
    'csi_article_event' AS source_table,
...
LEFT JOIN mis_db.ext_mdm_office_master emom on (toString(emom.office_id) =be.event_ofc_facility_id)

select * from mis_db.ext_mdm_office_master emom

ALTER TABLE tracking_event_mv ON CLUSTER cluster_1S_2R
DELETE WHERE event_date = toDateTime('1970-01-01 05:30:00')

ALTER TABLE tracking_event_mv
DELETE WHERE event_date = toDateTime64('1970-01-01 05:30:00', 6)

ALTER TABLE tracking_event_mv
DROP PARTITION tuple(toYYYYMM(toDateTime('1970-01-01')))

SELECT count() 
FROM tracking_event_mv 
WHERE event_date = toDateTime('1970-01-01 05:30:00')
